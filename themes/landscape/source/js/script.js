  ...
  $(".search-box-input").on("change", function (e) {
    ...
    if (value) {
      ...
      const matchedPostList = posts
        .filter((post) => {
          return post.raw.includes(value);
        })
        .map((post) => {
          ...
          post.matchedContent = `<div class="matched-content">${content.slice(
      ...
      if (matchedPostList.length) {
        $(".search-result-box").html(matchedPostList);
        $(".search-result-box").attr("style", "visibility:visible");
      } else {
        $(".search-result-box").html(`<div>暂无内容</div>`);
